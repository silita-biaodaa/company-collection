...
@Component
@JobHander(value = "HuNanSurveyCompanyUpdateTask")
public class HuNanSurveyCompanyUpdateTask extends BaseTask {

    @Override
    public void runTask(JSONObject jsonObject) throws Exception {
        surveyCompanyUpdateTask();
    }
    ...
    public void surveyCompanyUpdateTask() {
        ...
        for (Map<String, Object> map : maps) {
            ...
            for (int i = 0; i < threadCount; i++) {
                new Thread(new HuNanSurveyCompanyUpdateRun(i * every, (i + 1) * every, latch, urls)).start();
            }
            try {
                latch.await();
    ...
    class HuNanSurveyCompanyUpdateRun implements Runnable {
        ...
        public HuNanSurveyCompanyUpdateRun(int startNum, int endNum, CountDownLatch latch, List<String> CompanyQualificationUrls) {
